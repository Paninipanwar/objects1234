...
function draw() {
  ...
  for (let i = 0; i < detections.length; i++) {
    ...
    strokeWeight(4);
    noFill();
    rect(object.x + 10, object.y + 90, object.width + 10, object.height + 40);
    noStroke();
    fill(255);
    textSize(18);

    if (object.label === 'bottle') {
      text('bisleri', object.x + 60, object.y + 24);
      image(bisleriImage, -50, 440, bisleriImage.width / 3.4, bisleriImage.height / 3.4);
    } else if (object.label === 'banana') {
      text('banana', object.x + 60, object.y + 24);
      image(bananaImage, -50, 440, bananaImage.width / 3.4, bananaImage.height / 3.4);
    } else if (object.label === 'orange') {
      text('orange', object.x + 60, object.y + 24);
      image(orangeImage, -50, 440, orangeImage.width / 3.4, orangeImage.height / 3.4);
    } else {
